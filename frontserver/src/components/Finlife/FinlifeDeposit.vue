  ...
  <div>
    ...
    <div v-if="!searchstate" class="table-container">
      <table class="table" style="white-space: nowrap; width:1350px;">
        ...
        <tbody>
        <tr v-for="(item, idx) in depositlist" :key="item.id">
          <td>{{ idx + 1 }}</td>
          <td @click="showProductDetail(item)" style="cursor:pointer;"><a>{{ item.상품정보.fin_prdt_nm }}</a></td>
    ...
    <div>
      <b-modal v-model="showModal" title="상품 정보" >
        ...
        <template #modal-footer>
          <b-button variant="danger" @click="addProductDeposit(selectedProduct.상품정보.fin_prdt_nm, selectedProduct.상품정보.kor_co_nm)">등록</b-button>
  ...
  methods:{
    makeSearchList(){
      ...
      for (let i=0; i<this.depositlist.length; i++){
        const product_joinway = this.depositlist[i].상품정보.join_way.split(',')
        const product_saveterm = Object.keys(this.depositlist[i]).filter(key => this.depositlist[i][key] !== '상품정보없음');
        const product_bank = this.depositlist[i].상품정보.kor_co_nm
        ...
        if (this.selectedsaveterm.length != 0){
        ...
        if (joinway_bool && saveterm_bool && bank_bool){
          this.searchlist.push(this.depositlist[i])
        }
