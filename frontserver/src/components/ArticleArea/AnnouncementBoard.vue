    ...
    <div v-if="!searchState" class="table-container">
      <table class="table" style="white-space: nowrap; width:1350px;">
        ...
        <tr v-for="(article, idx) in articles" :key="article.id">
          <td>{{ idx + 1 }}</td>
          <td>공지</td>
          <td ><router-link :to="`/community/announce/${ article.id }`"><a style="color:black;">{{article.title}}</a></router-link></td>
...
export default {
  ...
  methods:{
    ...
    searchArtcle(){
      ...
      const articles = this.$store.state.announcementboard.anno_articles
