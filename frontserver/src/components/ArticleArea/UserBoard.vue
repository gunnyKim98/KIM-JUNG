<template>
  <div class="container">
    <h3 style="margin:10px;">유저게시판</h3>
    
      <div style="white-space: nowrap; display: flex;">
        <div style="flex: 1;">
          <button class="btn btn-outline-dark" @click="$router.push('/community/announce')">공지사항</button>
          <button class="btn btn-outline-dark" @click="$router.push('/community/newsboard')">뉴스게시판</button>
          <button disabled class="btn btn-outline-dark" @click="$router.push('/community/userboard')">유저게시판</button>
        </div>
    ...
    <div v-if="!searchState" class="table-container">
      <table class="table" style="white-space: nowrap; width:1350px;">
        ...
        <tr v-for="(article, idx) in articles" :key="article.id">
          ...
          <td ><router-link :to="`/community/userboard/${ article.id }`"><a style="color:black;">{{article.title}}</a></router-link></td>
...
export default {
  ...
  methods:{
    ...
    searchArtcle(){
      ...
      const articles = this.$store.state.userboard.articles
